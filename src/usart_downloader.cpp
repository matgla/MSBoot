...
void UsartDownloader::writeToMemory()
{
    ...
    softwareSize = msg.payload[0] << 24;
    softwareSize += msg.payload[1] << 16;
    softwareSize += msg.payload[2] << 8;
    softwareSize += msg.payload[3];
    logger_ << Level::INFO << "Received soft size: " << softwareSize << ":" << msg.payload[3] << "\r\n";
    ...
    while (receivedSize < softwareSize)
    {
        ...
        memcpy(page + receivedSize, msg1.payload, msg1.size);
